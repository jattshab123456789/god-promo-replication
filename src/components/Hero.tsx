...
import { useSiteContent, useContactInfo } from "@/hooks/useSiteContent";
...
const Hero = () => {
  ...
  const badge = content?.hero_badge || "Remote Video Editing Agency";
  const heading1 = content?.hero_heading_1 || "Grab more audience";
  const heading2 = content?.hero_heading_2 || "Using ";
  const heading3 = content?.hero_heading_3 || "our banger content";
  const subheading = content?.hero_subheading || "We partner with growth-focused businesses, providing Done-For-You organic content systems that drive leads effortlessly.";
      ...
      <div className="container relative z-10 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6 animate-slide-up">
            <span className="text-muted-foreground">{heading1}</span>
            <br />
            <span className="text-foreground">{heading2}</span>
            <span className="text-gradient">{heading3}</span>
          </h1>
